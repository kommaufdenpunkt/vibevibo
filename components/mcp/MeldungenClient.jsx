...
// 🚩 Meldungen (mcp_reports) — kategorisiert + Pull/Lock-Workflow.
// Filter-Tabs: Alle + alle real vorkommenden Kategorien (📷 Fotos, 💬 Beleidigung …).
// Pro Meldung: Übernehmen (claim) → Erledigen (resolve) / Freigeben (release).

import { useCallback, useEffect, useMemo, useState } from "react";

const CAT = {
  beleidigung: { e: "💬", label: "Beleidigung" },
  belaestigung: { e: "⚠️", label: "Belästigung" },
  voice: { e: "🎤", label: "Voice" },
  sprache: { e: "🎤", label: "Voice" },
  bild: { e: "📷", label: "Fotos" },
  foto: { e: "📷", label: "Fotos" },
  fotos: { e: "📷", label: "Fotos" },
  image: { e: "📷", label: "Fotos" },
...
export default function MeldungenClient() {
  ...
  // Kategorie-Tabs dynamisch aus den echten Daten + Zähler.
  const cats = useMemo(() => {
    const counts = {};
    ...
    return Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([k, n]) => ({ key: k, n, ...catInfo(k) }));
  }, [reports]);
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto", padding: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.85)", marginTop: 2 }}>
              Übernehmen (🔒 sperrt für andere) → prüfen → erledigen.
            </div>
          </div>
          <span style={{
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 8, marginBottom: 10 }}>
        ...
        {cats.map((c) => (
          <Chip key={c.key} active={cat === c.key} onClick={() => setCat(c.key)} label={`${c.label} ${c.n}`} emoji={c.e} />
        ))}
      </div>
      ...
      {data && visible.length === 0 && !err && (
        <div style={{ textAlign: "center", padding: 40, borderRadius: 16, background: "rgba(255,255,255,0.05)", border: "1px dashed rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.7)" }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 15, color: "#fff" }}>Keine Meldungen{cat !== "__all" ? " in dieser Kategorie" : ""}</div>
          <div style={{ fontSize: 12.5, marginTop: 4 }}>Sauber!</div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {visible.map((r) => {
          ...
          return (
            <div key={r.id} style={{
              background: "rgba(255,255,255,0.06)", border: `1px solid ${claimedByMe ? "rgba(34,197,94,0.45)" : "rgba(255,255,255,0.12)"}`,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11.5, fontWeight: 800, color: "#fff", background: "rgba(124,58,237,0.3)", border: "1px solid rgba(167,139,250,0.4)", borderRadius: 999, padding: "2px 10px" }}>
                  {ci.e} {ci.label}
                </span>
              ...
              {r.contentSnapshot && (
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.9)", background: "rgba(0,0,0,0.22)", borderRadius: 10, padding: "9px 11px", marginBottom: 8, lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
                  {r.contentSnapshot}
                </div>
              )}
              {r.reason && (
                <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.78)", marginBottom: 8 }}>
                  <b style={{ color: "#fca5a5" }}>Grund:</b> {r.reason}
                </div>
              )}

              <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.55)", marginBottom: 12, display: "flex", gap: 10, flexWrap: "wrap" }}>
                <span>Melder: <b style={{ color: "rgba(255,255,255,0.8)" }}>@{r.reporterUsername || r.reporterId}</b>{r.reporterGender === "f" ? " ♀" : r.reporterGender === "m" ? " ♂" : ""}</span>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {r.status === "open" && (
                  <button type="button" disabled={busy} onClick={() => act(r.id, "claim")} style={btn("linear-gradient(135deg, #7c3aed, #5b21b6)")}>
                    🔒 Übernehmen
                  </button>
                )}
                {claimedByMe && (
                  <>
                    <button type="button" disabled={busy} onClick={() => act(r.id, "resolve")} style={btn("linear-gradient(135deg, #16a34a, #15803d)")}>
                      ✅ Erledigt
                    </button>
                    <button type="button" disabled={busy} onClick={() => act(r.id, "release")} style={{
                      ...btn("rgba(255,255,255,0.08)"), color: "rgba(255,255,255,0.8)", border: "1px solid rgba(255,255,255,0.2)", flex: "0 0 auto",
                    }}>↩ Freigeben</button>
                  </>
                )}
                {claimedByOther && (
                  <div style={{ flex: 1, textAlign: "center", padding: "10px", fontSize: 12.5, color: "rgba(255,255,255,0.5)", fontWeight: 700 }}>
                    In Bearbeitung durch @{r.claimedByUsername || "anderen Mod"}
                  </div>
                )}
              </div>
            </div>
...
function Chip({ active, onClick, label, emoji }) {
  return (
    <button type="button" onClick={onClick} style={{
      ...
      background: active ? "linear-gradient(135deg, #ec4899, #7c3aed)" : "rgba(255,255,255,0.07)",
      color: active ? "#fff" : "rgba(255,255,255,0.7)",
      border: `1px solid ${active ? "rgba(244,114,182,0.5)" : "rgba(255,255,255,0.12)"}`,
    }}>{emoji} {label}</button>
  );
}
...
function btn(bg) {
  return {
    ...
    background: bg, color: "#fff", fontWeight: 800, fontSize: 13, fontFamily: "inherit",
  };
}
