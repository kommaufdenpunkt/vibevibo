...
import { getUserByUsername, listProfilePics, addProfilePic, countProfilePics, MAX_PROFILE_PICS, logMod } from "@/lib/db";
...
export async function GET(req, { params }) {
  ...
  return NextResponse.json({ pics, max: MAX_PROFILE_PICS, owner });
}
...
export async function POST(req, { params }) {
  ...
  if (countProfilePics(me.id) >= MAX_PROFILE_PICS) {
    return NextResponse.json({ error: `Maximal ${MAX_PROFILE_PICS} Profilbilder.` }, { status: 400 });
  }
