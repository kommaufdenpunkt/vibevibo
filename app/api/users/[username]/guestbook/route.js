...
export async function POST(req, { params }) {
  ...
  const cleaned = String(text || "").trim().slice(0, 600);
  ...
  const newId = addGuestbookEntry(target.id, me.id, cleaned);
  if (target.id !== me.id) {
    addNotification({ userId: target.id, actorId: me.id, type: "guestbook", targetType: "guestbook", targetId: newId, preview: cleaned });
  }
  notifyMentions(me.id, cleaned, "guestbook", newId);

  return NextResponse.json({ entries: getGuestbookEntries(target.id) });
}
