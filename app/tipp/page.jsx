...
// ⚽ WM-Tipp 2026 — voll integriert in VibeVibo, inkl. 1:1-Import von tipp.4ever1.tv.
//   • Spiele tippen (sperrt beim Anpfiff) · Punkte: exakt 4 · Tordifferenz 3 · Tendenz 2
//   • Bestenliste · pro Spiel „Tipps der Runde" (importiert) · Admin-Import-Button
...
function ptColor(p) {
  return p === 4 ? "#16a34a" : p === 3 ? "#1e40af" : p === 2 ? "#b45309" : "#6b7280";
}
  ...
  return (
    <div style={{ maxWidth: 760, margin: "16px auto", padding: 14 }}>
      <div style={{
        ...
        border: "3px ridge rgba(255,255,255,0.45)", boxShadow: "0 6px 20px rgba(30,64,175,0.35)",
      ...
      <div style={{
        ...
        background: "rgba(255,255,255,0.7)", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 1px 4px rgba(0,0,0,0.05)",
      }}>
          ...
          {board.length === 0 ? (
            ...
            board.map((u, i) => (
              <div key={u.id} style={{
                ...
                border: `1px solid ${i < 3 ? "rgba(249,115,22,0.4)" : "rgba(0,0,0,0.08)"}`, boxShadow: "0 1px 4px rgba(0,0,0,0.04)",
              }}>
                ...
                {avatarUrl(u.avatarUrl) && (
                  ...
                  <img src={avatarUrl(u.avatarUrl)} alt="" style={{ width: 30, height: 30, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
                )}
                {u.imported ? (
                  <div style={{ flex: 1, color: INK, fontWeight: 700, fontSize: 14, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {u.displayName}<span style={{ color: MUTED, fontSize: 11, fontWeight: 500 }}> · {u.bets} Spiele</span>
                  </div>
                ) : (
                  <Link href={`/u/${u.username}`} style={{ flex: 1, color: INK, textDecoration: "none", fontWeight: 700, fontSize: 14, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {u.displayName || u.username}<span style={{ color: MUTED, fontSize: 11, fontWeight: 500 }}> · {u.bets} Tipps</span>
                  </Link>
                )}
...
function Muted({ children }) {
  return (
    <div style={{ padding: 28, textAlign: "center", color: MUTED, fontSize: 14, lineHeight: 1.6, background: "rgba(255,255,255,0.7)", borderRadius: 14, border: "1px dashed rgba(0,0,0,0.1)" }}>
      {children}
    </div>
  );
}
...
function MatchCard({ m, bet, canTip, onSaved }) {
  ...
  return (
    <div style={{ marginBottom: 10, padding: 14, borderRadius: 14, background: "#ffffff", border: "1px solid rgba(0,0,0,0.08)", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginTop: 10, minHeight: 22, flexWrap: "wrap" }}>
        ...
        {!locked && canTip && (
          <button type="button" onClick={save} disabled={busy} style={{
            ...
            background: "linear-gradient(135deg, #1e40af, #3b82f6)", color: "#fff", fontWeight: 800, fontSize: 12.5,
            fontFamily: "inherit", boxShadow: "0 2px 6px rgba(30,64,175,0.3)",
      ...
      {tips.length > 0 && (
        <div style={{ marginTop: 8, borderTop: "1px solid rgba(0,0,0,0.06)", paddingTop: 8 }}>
          <button type="button" onClick={() => setShowTips((s) => !s)} style={{
            ...
            color: BLUE, fontSize: 12, fontWeight: 800, padding: 0,
          }}>
...
function scoreInput(locked) {
  return {
    ...
    border: `1.5px solid ${locked ? "rgba(0,0,0,0.12)" : "rgba(30,64,175,0.4)"}`,
    ...
    <div style={{ marginBottom: 14, borderRadius: 14, border: "1px solid rgba(249,115,22,0.45)", background: "#fffbeb", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      ...
      {open && (
        <div style={{ padding: "0 14px 14px" }}>
          {/* IMPORT */}
          <div style={{ marginBottom: 12, padding: 12, borderRadius: 10, background: "#fff", border: "1px solid rgba(0,0,0,0.08)" }}>
            ...
            <button type="button" onClick={runImport} disabled={importing} style={{
              ...
              background: "linear-gradient(135deg, #1e40af, #3b82f6)", color: "#fff", fontWeight: 800, fontSize: 13, fontFamily: "inherit",
            }}>{importing ? "⏳ Importiere …" : "⬇️ Jetzt von 4ever1 importieren"}</button>
            {importMsg && <div style={{ marginTop: 8, fontSize: 12, fontWeight: 700, color: importMsg.startsWith("⚠") ? "#b91c1c" : importMsg.startsWith("✅") ? "#15803d" : MUTED }}>{importMsg}</div>}
          </div>
          ...
          {(matches || []).length > 0 && (
            <div style={{ marginTop: 6 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: MUTED, marginBottom: 4 }}>SPIELE VERWALTEN ({matches.length})</div>
