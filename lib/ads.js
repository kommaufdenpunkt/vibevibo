...
export function getDisplayConfig() {
  ...
  switch (provider) {
    ...
    case "adsterra": {
      const zoneId = cfg("ADSTERRA_ZONE_ID", "");
      const rawDomain = String(cfg("ADSTERRA_BANNER_DOMAIN", "www.highperformanceformat.com")).trim()
        .replace(/^https?:\/\//i, "").replace(/\/+$/, "");
      return {
        provider,
        zoneId,
        domain: rawDomain || "www.highperformanceformat.com",
        width: Math.max(50, Math.min(1200, Number(cfg("ADSTERRA_BANNER_WIDTH", 320)) || 320)),
        height: Math.max(30, Math.min(900, Number(cfg("ADSTERRA_BANNER_HEIGHT", 50)) || 50)),
        enabled: !!zoneId,
      };
    }
    case "off":
...
// 💰 Social Bar (Adsterra): sticky-Bar am Bildschirmrand, höchste CTR.
// Konfigurierbar via Admin (komplette Script-URL einsetzen).
export function getSocialBarConfig() {
  const url = String(cfg("ADSTERRA_SOCIALBAR_SCRIPT_URL", "")).trim();
  const enabled = !!url && /^https:\/\//i.test(url);
  return { enabled, scriptUrl: enabled ? url : "" };
}

// 💰 In-Page Push (Adsterra): Pseudo-Push-Notifications in-page.
// Funktioniert auch ohne dass User Browser-Push erlaubt hat.
export function getInPagePushConfig() {
  const url = String(cfg("ADSTERRA_INPAGEPUSH_SCRIPT_URL", "")).trim();
  const enabled = !!url && /^https:\/\//i.test(url);
  return { enabled, scriptUrl: enabled ? url : "" };
}
...
export function getProviderConfig() {
  ...
  return {
    ...
    socialBar: getSocialBarConfig(),
    inPagePush: getInPagePushConfig(),
  };
}
