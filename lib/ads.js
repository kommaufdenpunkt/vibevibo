...
export const DISPLAY_PROVIDERS = [
  ...
  { id: "adsense",  name: "Google AdSense", description: "Premium-Reichweite, beste Auszahlung — braucht Genehmigung von Google.", signupUrl: "https://www.google.com/adsense/start/" },
...
export function getDisplayConfig() {
  ...
  switch (provider) {
    case "adsense": {
      const pubId = cfg("ADSENSE_PUB_ID", "");
      return {
        provider,
        // Google AdSense Auto-Ads Loader-URL
        scriptUrl: pubId ? `https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${encodeURIComponent(pubId)}` : "",
        pubId,
        autoAds: cfg("ADSENSE_AUTO_ADS", "1") !== "0",
        enabled: !!pubId,
      };
    }
    case "ezoic":
...
export function getProviderConfig() {
  ...
  return {
    ...
    // Liste der aktiven Earning-Provider fuer Reward-Hub
    earningProviders: listActiveEarningProvidersForClient(),
    // Display-Provider-Config (Ezoic / Adsterra / off)
