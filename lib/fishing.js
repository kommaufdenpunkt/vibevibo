...
export async function waterNearby(lat, lng, radiusM = 150) {
  ...
  const cached = _waterCache.get(key);
  if (cached && Date.now() - cached.at < WATER_TTL) return cached.water;
  ...
  try {
    ...
    const water = Array.isArray(d?.elements) && d.elements.length > 0;
    _waterCache.set(key, { at: Date.now(), water });
    return water;
  } catch {
    return null;
  } finally {
