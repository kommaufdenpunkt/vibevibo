...
export const api = {
  ...
  worldFish: (lat, lng, accuracy) => req("/api/world/fish", { method: "POST", body: JSON.stringify({ lat, lng, accuracy }) }),
  market: () => req("/api/market"),
  marketSell: (merchantId, ids) => req("/api/market/sell", { method: "POST", body: JSON.stringify({ merchantId, ids }) }),
