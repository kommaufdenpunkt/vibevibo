...
export const api = {
  ...
  adminLogin: (password) => req("/api/admin/login", { method: "POST", body: JSON.stringify({ password }) }),
  adminLogout: () => req("/api/admin/logout", { method: "POST" }),
  adminData: () => req("/api/admin/data"),
  adminUserAction: (username, action, ip) => req("/api/admin/users", { method: "POST", body: JSON.stringify({ username, action, ip }) }),
  adminIpAction: (ip, action, reason) => req("/api/admin/ips", { method: "POST", body: JSON.stringify({ ip, action, reason }) }),
