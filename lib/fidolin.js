...
const IMAGE_PROMPT = `Du bist Fidolin und prüfst ein Profilbild für eine nostalgische Community (Alter 14+).
Lehne ab bei: Nacktheit/Sexuelles, Gewalt/Blut, Hass-Symbolen (z.B. Hakenkreuz),
Drogen, oder offensichtlich nicht jugendfreien Inhalten. Normale Selfies, Tiere,
Landschaften, Avatare, Cartoons sind ERLAUBT.
Antworte AUSSCHLIESSLICH als JSON:
...
export async function moderateImage(dataUrl) {
  ...
  if (!fidolinEnabled()) {
    // Ohne KI kann Fidolin Bilder nicht prüfen -> Mensch entscheidet
    return { block: false, undecided: true, reason: "Keine KI – wartet auf Admin", by: "fidolin" };
  }
  try {
    ...
    const j = parseJsonLoose(out);
    if (!j || typeof j.block !== "boolean") return { block: false, undecided: true, reason: "KI unklar – wartet auf Admin", by: "fidolin" };
    return { block: !!j.block, reason: String(j.reason || "").slice(0, 200), by: "fidolin", undecided: false };
  } catch {
    return { block: false, undecided: true, reason: "KI nicht erreichbar – wartet auf Admin", by: "fidolin" };
  }
}
