...
// Fidolin Auto-Run: bei User-Traffic alle ~22h selbst checken (kein externer Cron noetig).
const FIDOLIN_INTERVAL_MS = 22 * 60 * 60 * 1000;

export function maybeRunFidolinCheck() {
  try {
    const last = Number(getSetting("ECONOMY_LAST_CHECK", "0")) || 0;
    if (Date.now() - last < FIDOLIN_INTERVAL_MS) return null;
    return runFidolinEconomyCheck("fidolin-auto");
  } catch {
    return null;
  }
}
