...
export function getProviderConfig(provider) {
  ...
  if (provider === "google") {
    return {
      provider: "google",
      clientId:     readCfg("GOOGLE_CLIENT_ID"),
      clientSecret: readCfg("GOOGLE_CLIENT_SECRET"),
...
export async function exchangeCodeForToken(provider, code, redirectUri) {
  ...
  if (provider === "google") {
    ...
    const json = await r.json();
    if (!r.ok) throw new Error(json?.error_description || json?.error || "Google OAuth fehler");
    return {
      accessToken:  json.access_token,
      expiresIn:    json.expires_in || 0,
      refreshToken: json.refresh_token || "",
...
export async function fetchUserProfile(provider, accessToken) {
  ...
  if (provider === "google") {
    ...
    const json = await r.json();
    if (!r.ok) throw new Error(json?.error?.message || "Google profile fehler");
    return {
      providerUserId: String(json.sub || ""),
      displayName:    json.name || json.given_name || "",
      email:          json.email || "",
