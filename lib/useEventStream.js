...
export function useMessageStream(enabled, handlers) {
  ...
  useEffect(() => {
    ...
    es.addEventListener("call-incoming", wrap("onCallIncoming"));
    es.addEventListener("call-peer-joined", wrap("onCallPeerJoined"));
    es.addEventListener("call-peer-left", wrap("onCallPeerLeft"));
    es.addEventListener("call-ended", wrap("onCallEnded"));
