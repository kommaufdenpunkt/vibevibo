...
export function createLiveStream({ ownerId, title, mode = "solo", hasVideo = true, hasAudio = true, maxHosts = 1 }) {
  const now = Date.now();
  ...
  const tx = db().transaction(() => {
    const r = db().prepare(`
      INSERT INTO live_streams (owner_id, title, mode, has_video, has_audio, max_hosts, status, started_at)
      VALUES (?, ?, ?, ?, ?, ?, 'live', ?)
    `).run(ownerId, String(title || "").slice(0, 100), mode, hasVideo ? 1 : 0, hasAudio ? 1 : 0, Number(maxHosts), now);
...
export function joinLiveHost(streamId, userId) {
  const s = db().prepare("SELECT mode, max_hosts AS maxHosts, status FROM live_streams WHERE id = ?").get(streamId);
  if (!s) throw new Error("Stream nicht gefunden.");
  ...
  const now = Date.now();
  // Falls schon mal Host war, neu anlegen
