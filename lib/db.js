...
function migrate(d) {
  ...
  addColumnIfMissing(d, "users", "premium_badges", "TEXT DEFAULT ''"); // CSV: gold,diamond,rainbow,frame_neon,frame_gold,vanity,bio_xl,invisible
...
export function buyPremium(userId, kind, payload = {}) {
  ...
  if (kind === "username_change" || kind === "username_change_fast") {
    ...
    // 1-Jahr-Sperre nur beim "normalen" Wechsel
...
export function useDisplayNameCredit(userId, newDisplayName) {
  const newDn = String(newDisplayName || "").trim();
  if (newDn.length < 1 || newDn.length > 40) throw new Error("Anzeigename 1-40 Zeichen.");
  const exists = db().prepare("SELECT 1 FROM users WHERE LOWER(display_name) = LOWER(?) AND id != ?").get(newDn, userId);
  if (exists) throw new Error("Anzeigename ist bereits vergeben.");
  const row = db().prepare("SELECT displayname_credits FROM users WHERE id = ?").get(userId);
