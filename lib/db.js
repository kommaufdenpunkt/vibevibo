...
export function addStatusUpdate(userId, text) {
  const t = String(text || "").trim().slice(0, 60);
  if (!t) return;
  db().prepare("INSERT INTO status_updates (user_id, text, created_at) VALUES (?, ?, ?)").run(userId, t, Date.now());
}
...
export function getBuschfunk(limit = 30) {
  ...
  for (const s of d.prepare(`
    SELECT s.text AS detail, s.created_at AS at,
  ...
  `).all(limit)) {
    events.push({ type: "status", at: s.at, detail: s.detail,
