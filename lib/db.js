...
function migrate(d) {
  ...
  addColumnIfMissing(d, "users", "premium_badges", "TEXT DEFAULT ''"); // CSV: gold,diamond,rainbow
...
export function tickAndPersistVibo(userId) {
  ...
  const updated = { ...v };
  updated.hunger    = viboClamp(v.hunger    - hours * 4 * decayFactor);
  updated.fun       = viboClamp(v.fun       - hours * 3 * decayFactor);
  updated.hygiene   = viboClamp(v.hygiene   - hours * 2 * decayFactor);
  updated.affection = viboClamp(v.affection - hours * 1 * decayFactor);
  ...
  db().prepare(`
    UPDATE vibos SET hunger=?, fun=?, hygiene=?, affection=?, health=?, last_tick_at=?,
      died_at=?, death_reason=?, last_birthday_at=?
    WHERE user_id=?
  `).run(
    ...
    updated.last_birthday_at || v.last_birthday_at || 0, userId
  );
...
export function buyPremium(userId, kind, payload = {}) {
  ...
  // Spezial-Validierungen vor Bezahlung
  if (kind === "username_change") {
    const newName = String(payload?.newUsername || "").trim().toLowerCase();
    if (!/^[a-z0-9_]{3,20}$/.test(newName)) throw new Error("Username 3-20 Zeichen (a-z, 0-9, _).");
    ...
    if (me?.last_username_change_at && Date.now() - me.last_username_change_at < 365 * 24 * 3600_000) {
      const days = Math.ceil((365 * 24 * 3600_000 - (Date.now() - me.last_username_change_at)) / (24 * 3600_000));
      throw new Error(`Erst wieder in ${days} Tagen änderbar.`);
    }
  }
  if (kind === "displayname_change") {
    const newDn = String(payload?.newDisplayName || "").trim();
    if (newDn.length < 1 || newDn.length > 40) throw new Error("Anzeigename 1-40 Zeichen.");
    // Anzeigename muss unique sein (case-insensitive)
  ...
  if (kind === "badge_gold" || kind === "badge_diamond" || kind === "frame_rainbow") {
    const tag = kind.replace("badge_", "").replace("frame_", "frame_");
    const existing = getPremiumBadges(userId);
    const slug = kind === "badge_gold" ? "gold" : kind === "badge_diamond" ? "diamond" : "rainbow";
    if (existing.includes(slug)) throw new Error("Hast du schon.");
  }
  ...
  } else if (kind === "custom_status") {
    db().prepare("UPDATE users SET mood = ? WHERE id = ?").run(payload.text, userId);
    note = "Status gesetzt";
  } else if (kind === "displayname_change") {
    db().prepare("UPDATE users SET display_name = ? WHERE id = ?").run(payload.newDisplayName.trim(), userId);
    note = "Anzeigename geändert";
  } else if (kind === "username_change") {
    db().prepare("UPDATE users SET username = ?, last_username_change_at = ? WHERE id = ?")
      .run(payload.newUsername.trim().toLowerCase(), Date.now(), userId);
    note = "Username geändert — bitte neu einloggen.";
  } else if (kind === "badge_gold") {
    setPremiumBadges(userId, [...getPremiumBadges(userId), "gold"]);
    note = "Gold-Badge freigeschaltet 🥇";
  } else if (kind === "badge_diamond") {
    setPremiumBadges(userId, [...getPremiumBadges(userId), "diamond"]);
    note = "Diamant-Badge freigeschaltet 💎";
  } else if (kind === "frame_rainbow") {
    setPremiumBadges(userId, [...getPremiumBadges(userId), "rainbow"]);
    note = "Regenbogen-Rahmen aktiviert 🌈";
  } else if (kind === "status_slot") {
    // Stub — kommt in einer späteren Iteration, jetzt nur als Note
    note = "Status-Slot vorgemerkt (Feature folgt)";
  }
...
export function getBuschfunk(limit = 30) {
  ...
  for (const p of d.prepare(`
    SELECT p.created_at AS at, f.username AS au, f.display_name AS an, f.gender AS ag, f.birthdate AS abd, f.last_seen AS als,
  ...
  `).all(limit)) {
    events.push({ type: "pinnwand", at: p.at, actor: { username: p.au, displayName: p.an, gender: p.ag, age: ageFromBirthdate(p.abd), lastSeen: p.als },
      target: { username: p.tu, displayName: p.tn }, detail: p.detail });
  }

  for (const g of d.prepare(`
    SELECT g.created_at AS at, g.gift_id AS gift, f.username AS au, f.display_name AS an, f.gender AS ag, f.birthdate AS abd, f.last_seen AS als,
  ...
  `).all(limit)) {
    events.push({ type: "gift", at: g.at, gift: g.gift, actor: { username: g.au, displayName: g.an, gender: g.ag, age: ageFromBirthdate(g.abd), lastSeen: g.als },
      target: { username: g.tu, displayName: g.tn } });
  }

  for (const gp of d.prepare(`
    SELECT gp.created_at AS at, u.username AS au, u.display_name AS an, u.gender AS ag, u.birthdate AS abd, u.last_seen AS als,
  ...
  `).all(limit)) {
    events.push({ type: "grouppost", at: gp.at, actor: { username: gp.au, displayName: gp.an, gender: gp.ag, age: ageFromBirthdate(gp.abd), lastSeen: gp.als },
      group: { slug: gp.gslug, name: gp.gname }, detail: gp.detail });
  }

  for (const u of d.prepare(`
    SELECT created_at AS at, username AS au, display_name AS an, gender AS ag, birthdate AS abd, last_seen AS als
      FROM users ORDER BY created_at DESC LIMIT 10
  `).all()) {
    events.push({ type: "newuser", at: u.at, actor: { username: u.au, displayName: u.an, gender: u.ag, age: ageFromBirthdate(u.abd), lastSeen: u.als } });
  }

  for (const p of d.prepare(`
    SELECT p.id AS picId, p.url AS picUrl, p.created_at AS at,
           u.username AS au, u.display_name AS an, u.gender AS ag, u.birthdate AS abd, u.last_seen AS als
      FROM profile_pics p JOIN users u ON u.id = p.user_id
     WHERE p.status = 'approved' ORDER BY p.created_at DESC LIMIT ?
  `).all(limit)) {
    events.push({ type: "newpic", at: p.at, picId: p.picId, picUrl: p.picUrl,
      actor: { username: p.au, displayName: p.an, gender: p.ag, age: ageFromBirthdate(p.abd), lastSeen: p.als } });
  }

  for (const s of d.prepare(`
    SELECT s.text AS detail, s.image_url AS picUrl, s.created_at AS at,
           u.username AS au, u.display_name AS an, u.emoji AS ae, u.gender AS ag, u.birthdate AS abd, u.last_seen AS als
  ...
  `).all(limit)) {
    events.push({ type: "status", at: s.at, detail: s.detail, picUrl: s.picUrl || "",
      actor: { username: s.au, displayName: s.an, emoji: s.ae, gender: s.ag, age: ageFromBirthdate(s.abd), lastSeen: s.als } });
  }
