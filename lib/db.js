...
function migrate(d) {
  d.exec(`
    ...
    -- 🚫 Hard-Block zwischen 2 Usern: kein Chat, keine Pinnwand, keine
    -- Geschenke, keine Komplimente, keine Visit, keine Anfragen. Beidseitig
    -- wirksam: wenn A B blockt, sehen sich beide nicht und koennen sich nicht
    -- kontaktieren — egal welche Richtung.
    ...
    -- 🔗 Verknuepfte Social-Accounts (Facebook/Instagram/Snapchat)
    -- access_token wird verschluesselt gespeichert (vv-token: AES-GCM Hex)
    -- raw_profile haelt das letzte abgerufene Roh-Profil als JSON fest
    CREATE TABLE IF NOT EXISTS linked_accounts (
      user_id           INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      provider          TEXT NOT NULL,           -- 'facebook' | 'instagram' | 'snapchat'
      provider_user_id  TEXT NOT NULL,
      display_name      TEXT DEFAULT '',
      avatar_url        TEXT DEFAULT '',
      access_token_enc  TEXT DEFAULT '',         -- AES-GCM verschluesselt
    ...
    -- 🔐 Temporaere OAuth-State-Tokens (CSRF-Schutz im Callback)
    CREATE TABLE IF NOT EXISTS oauth_states (
      state         TEXT PRIMARY KEY,
      provider      TEXT NOT NULL,
      user_id       INTEGER,                     -- bei Account-Linking: aktueller User
      created_at    INTEGER NOT NULL,
      next_url      TEXT DEFAULT '/profile'
    );
...
// ============================================================
// 🚫 Hard-Block zwischen Usern
// ============================================================

// Block setzen. Beidseitig wirksam — auch wenn nur A blockt B, blockiert
// das System auch B->A-Kontakt (sonst koennte Stalker dauernd neu zuschreiben).
// Selbst-Block + Block-Admin sind verboten.
export function addUserBlock(blockerId, blockedId, reason = "") {
  if (!blockerId || !blockedId || blockerId === blockedId) return false;
  db().prepare(`
    INSERT OR REPLACE INTO user_blocks (blocker_id, blocked_id, reason, created_at)
    VALUES (?, ?, ?, ?)
...
// Schnellpruefung: existiert ein Block IRGENDEINER Richtung zwischen A und B?
// Liefert true sobald A B blockt ODER B A blockt — fuer Enforcement.
...
// Nur die Richtung pruefen die der blocker explizit gesetzt hat — fuer UI
// (z.B. 'du hast diesen User blockiert' vs 'du bist von ihm blockiert').
...
// Liste alle User die ich blockiert habe (Settings-Seite)
export function listMyBlocks(userId) {
  return db().prepare(`
    SELECT b.blocked_id AS userId, b.reason, b.created_at AS createdAt,
           u.username, u.display_name AS displayName, u.emoji,
           u.avatar_url AS avatarUrl, u.avatar_status AS avatarStatus
      FROM user_blocks b
...
// Set aller User-IDs die mit mir in irgendeiner Block-Beziehung stehen —
// fuer Liste-Filter (Freunde, Mitglieder, Karte etc).
export function blockedUserIdsFor(userId) {
  const rows = db().prepare(`
    SELECT CASE WHEN blocker_id = ? THEN blocked_id ELSE blocker_id END AS other
      FROM user_blocks
...
// ============================================================
// (crypto wird oben in der Datei bereits importiert)

// AES-GCM Verschluesselung mit Key aus VV_TOKEN_KEY (oder VV_ADMIN_PASSWORD-Hash als Fallback)
function tokenKey() {
  const raw = process.env.VV_TOKEN_KEY || (process.env.VV_ADMIN_PASSWORD || "vv-fallback-key");
  return crypto.createHash("sha256").update("vv-token-key:" + raw).digest();
}
...
export function decryptToken(hex) {
  if (!hex) return "";
  try {
    ...
    const dec = crypto.createDecipheriv("aes-256-gcm", tokenKey(), iv);
    dec.setAuthTag(tag);
...
export function createOAuthState(provider, userId = null, nextUrl = "/profile") {
  const state = crypto.randomBytes(24).toString("hex");
  db().prepare(`
    INSERT INTO oauth_states (state, provider, user_id, created_at, next_url)
    VALUES (?, ?, ?, ?, ?)
  `).run(state, provider, userId || null, Date.now(), String(nextUrl || "/profile").slice(0, 200));
  return state;
}
export function consumeOAuthState(state) {
  const row = db().prepare("SELECT * FROM oauth_states WHERE state = ?").get(state);
  if (!row) return null;
  // 10 Minuten Gueltigkeit
  if (Date.now() - row.created_at > 10 * 60_000) {
...
// Aufraeumen alter States
export function cleanupOAuthStates() {
  db().prepare("DELETE FROM oauth_states WHERE created_at < ?").run(Date.now() - 24 * 3600_000);
}
...
export function upsertLinkedAccount(userId, provider, data) {
  ...
  db().prepare(`
    INSERT INTO linked_accounts
      (user_id, provider, provider_user_id, display_name, avatar_url,
       access_token_enc, refresh_token_enc, expires_at, scope, raw_profile,
  ...
  `).run(
    userId, provider, String(data.providerUserId || ""),
    String(data.displayName || ""), String(data.avatarUrl || ""),
...
export function findUserByLinkedAccount(provider, providerUserId) {
  const row = db().prepare(
    "SELECT user_id FROM linked_accounts WHERE provider = ? AND provider_user_id = ?"
  ).get(provider, String(providerUserId || ""));
  return row ? row.user_id : null;
}

export function getLinkedAccount(userId, provider) {
  const row = db().prepare(
  ...
  return {
    ...
    scope: row.scope,
    rawProfile: (() => { try { return JSON.parse(row.raw_profile); } catch { return {}; } })(),
...
export function listLinkedAccounts(userId) {
  return db().prepare(`
    SELECT provider, provider_user_id AS providerUserId, display_name AS displayName,
           avatar_url AS avatarUrl, scope, linked_at AS linkedAt, last_synced_at AS lastSyncedAt
      FROM linked_accounts WHERE user_id = ? ORDER BY linked_at DESC
  `).all(userId);
}
