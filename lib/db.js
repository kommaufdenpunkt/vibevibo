...
function migrate(d) {
  ...
  // 💬 Buschfunk-Kommentare (mit Reply, Soft-Delete, KI/Admin-Moderation)
  d.exec(`
...
// Kommentar hinzufügen. text wird vom Caller schon Fidolin-geprüft.
// replyToId: 0 für Top-Level, sonst die comment.id auf die geantwortet wird.
export function addBuschfunkComment(postId, userId, text, replyToId = 0) {
  const t = String(text || "").trim().slice(0, 500);
  if (!t) throw new Error("Kommentar leer.");
  // Existiert der Post noch?
  const post = db().prepare("SELECT id FROM status_updates WHERE id = ?").get(postId);
  if (!post) throw new Error("Buschfunk-Post nicht gefunden.");
  // Reply-To muss zum gleichen Post gehören und nicht gelöscht sein
  let r2 = 0;
  if (replyToId) {
    const parent = db().prepare(
      "SELECT post_id, deleted_at FROM buschfunk_comments WHERE id = ?"
    ).get(replyToId);
    if (parent && parent.post_id === Number(postId) && !parent.deleted_at) r2 = Number(replyToId);
  }
  const info = db().prepare(
    "INSERT INTO buschfunk_comments (post_id, user_id, text, reply_to_id, created_at) VALUES (?, ?, ?, ?, ?)"
  ).run(postId, userId, t, r2, Date.now());
  return info.lastInsertRowid;
}

// Liste aller Kommentare unter einem Post — flach mit reply_to_id.
// Gelöschte werden als Platzhalter zurückgegeben (für Thread-Struktur), Inhalt entfernt.
export function listBuschfunkComments(postId, { byUserId } = {}) {
  return db().prepare(`
    SELECT c.id, c.user_id, c.text, c.reply_to_id, c.created_at AS at,
