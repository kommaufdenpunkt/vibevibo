...
export function getBuschfunk(limit = 30) {
  ...
  for (const p of d.prepare(`
    SELECT p.created_at AS at, f.username AS au, f.display_name AS an, f.gender AS ag, f.birthdate AS abd, f.last_seen AS als, f.premium_badges AS abadges,
  ...
  `).all(limit)) {
    events.push({ type: "pinnwand", at: p.at, actor: { username: p.au, displayName: p.an, gender: p.ag, age: ageFromBirthdate(p.abd), lastSeen: p.als, premiumBadges: parseBadges(p.abadges) },
      target: { username: p.tu, displayName: p.tn }, detail: p.detail });
  }

  for (const g of d.prepare(`
    SELECT g.created_at AS at, g.gift_id AS gift, f.username AS au, f.display_name AS an, f.gender AS ag, f.birthdate AS abd, f.last_seen AS als, f.premium_badges AS abadges,
  ...
  `).all(limit)) {
    events.push({ type: "gift", at: g.at, gift: g.gift, actor: { username: g.au, displayName: g.an, gender: g.ag, age: ageFromBirthdate(g.abd), lastSeen: g.als, premiumBadges: parseBadges(g.abadges) },
