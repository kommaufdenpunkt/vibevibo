...
export function setRelationshipStatus(userId, status, partnerUsername = "") {
  const cleaned = String(status || "").trim();
  if (!RELATIONSHIP_STATUSES.has(cleaned)) throw new Error("Ungültiger Familienstand.");
  let partnerId = 0;
  if (cleaned && cleaned !== "single") {
    const name = String(partnerUsername || "").trim();
    if (name) {
      // Partner per Username oder Display-Name finden
      const partner = db().prepare(
        "SELECT id, relationship_status FROM users WHERE LOWER(username) = LOWER(?) OR LOWER(display_name) = LOWER(?)"
      ).get(name, name);
      if (!partner) throw new Error(`„${name}" wurde nicht gefunden.`);
      if (partner.id === userId) throw new Error("Du kannst dich nicht mit dir selbst verlinken 😄");
      // Konsens: Partner muss SCHON taken/engaged/married eingetragen haben
      // (sonst kann der Link missbraucht werden um Leute zu „outen").
      if (!PARTNER_REQUIRED_STATUSES.has(partner.relationship_status || "")) {
        throw new Error(
          `${name} hat noch keinen Beziehungsstatus gesetzt. ` +
          `${name} muss zuerst „vergeben/verlobt/verheiratet" auswählen, dann kannst du verlinken.`
        );
      }
      partnerId = partner.id;
    }
  }
  db().prepare("UPDATE users SET relationship_status = ?, partner_user_id = ? WHERE id = ?")
    .run(cleaned, partnerId, userId);
  return { status: cleaned, partnerUserId: partnerId };
}
...
// Anzeigename: 1-20 Zeichen, keine Leerzeichen, nur Buchstaben/Zahlen/_/-
export function validateDisplayName(name) {
  const v = String(name || "").trim();
  if (v.length < 1 || v.length > 20) throw new Error("Anzeigename: 1-20 Zeichen.");
...
export function getBuschfunk(limit = 30) {
  ...
  for (const s of d.prepare(`
    SELECT s.text AS detail, s.image_url AS picUrl, s.audio_url AS audioUrl,
