...
function db() {
  ...
  _db.pragma("busy_timeout = 10000"); // verhindert "SQLITE_BUSY" wenn mehrere next-build-Worker parallel migrieren
...
function migrate(d) {
  ...
  addColumnIfMissing(d, "users", "shield_enabled", "INTEGER DEFAULT 0");
  addColumnIfMissing(d, "users", "shield_msg_policy", "TEXT DEFAULT 'all'");
  addColumnIfMissing(d, "users", "shield_min_age_days", "INTEGER DEFAULT 0");
  addColumnIfMissing(d, "users", "shield_word_filter", "INTEGER DEFAULT 0");
...
function addColumnIfMissing(d, table, column, definition) {
  ...
  if (!cols.some((c) => c.name === column)) {
    try {
      ...
      // Parallele next-build-Worker können dieselbe Spalte gleichzeitig anlegen → ignorieren
      if (!String(e && e.message || e).includes("duplicate column name")) throw e;
    }
  }
}
...
// 📊 Öffentliche Stats für die Landing-Seite (cached pro Request).
export function getPublicStats() {
  const onlineSince = Date.now() - 90_000;
  const todayStart = new Date(); todayStart.setHours(0, 0, 0, 0);
  const since = todayStart.getTime();
  try {
    const users = db().prepare("SELECT COUNT(*) AS n FROM users WHERE status != 'banned'").get()?.n || 0;
    const online = db().prepare("SELECT COUNT(*) AS n FROM users WHERE last_seen > ? AND (presence != 'invisible' OR presence IS NULL)").get(onlineSince)?.n || 0;
    const pinnwand = db().prepare("SELECT COUNT(*) AS n FROM pinnwand").get()?.n || 0;
    const giftsToday = db().prepare("SELECT COUNT(*) AS n FROM gifts WHERE created_at > ?").get(since)?.n || 0;
    return { users, online, pinnwand, gifts: giftsToday };
  } catch {
    return null;
  }
}
...
// Admin-only Passwort-Reset: ersetzt password_hash direkt (bcrypt 12 rounds).
...
export function buyPremium(userId, kind, payload = {}) {
  ...
  // ---- Bezahlen ----
  const spend = spendCredits(userId, item.price, `premium:${kind}`, { type: "premium", id: 0 });
  if (!spend.ok) throw new Error(`Zu wenig Vibes (fehlen ${spend.missing} ✨).`);
  ...
  addToSink(Math.round(item.price * sinkShare));
...
// ============================================================
// Fidolin als System-User: begrüßt jeden neuen Mitglieder auf der Pinnwand
// ============================================================
export function ensureFidolinUser() {
  const existing = db().prepare("SELECT id FROM users WHERE username = ?").get("fidolin");
  if (existing) return existing.id;
  const now = Date.now();
  const hash = bcrypt.hashSync("__fidolin_system_no_login_" + now + "_" + Math.random(), 10);
  const info = db().prepare(`
    INSERT INTO users (username, display_name, password_hash, emoji, mood, about_me, interests, bg_music, created_at, last_seen, status, reg_ip, gender, birthdate)
    VALUES (?, ?, ?, ?, ?, ?, '[]', '', ?, ?, 'approved', '', 'm', '2000-01-01')
  `).run(
    "fidolin", "Fidolin 🤖", hash, "🤖", "freundlich",
    "Ich bin Fidolin, die KI-Begleitung von VibeVibo. ✨ Ich begrüße neue Mitglieder, halte Ordnung und sorge dafür dass hier alle Spaß haben."
  , now, now);
  return info.lastInsertRowid;
}

const FIDOLIN_WELCOME_TEXTS = [
  "✨ Heyhey {name}! Schön dass du bei VibeVibo bist! Ich bin Fidolin 🤖 — eure KI-Begleitung. Klick auf 📌 Pinnwand bei anderen um Glitzer-Smileys zu verteilen, schick deinen ersten *Gruschel* oder verschick ne Rose. Wenn was hakt: ich bin immer erreichbar. Viel Spaß im neuen alten Internet! ✿",
  "🌟 Willkommen {name}! Ich bin Fidolin 🤖 und freu mich riesig dass du dabei bist! Erinnerst du dich an ICQ-Sounds, Mixtapes brennen und Top-5-Friends? Genau das findest du hier wieder. Schmück dein Profil aus, finde alte Bekannte wieder und vergiss nicht den 📣 Buschfunk-Feed zu checken! 💖",
  "💖 Hallo {name}! Schön dich kennenzulernen! Ich bin Fidolin 🤖 — die freundliche KI hier bei VibeVibo. Falls du Hilfe brauchst, jemand seltsam wird, oder du einfach Fragen hast: schreib mir. Ansonsten: pack Glitzer auf dein Profil, such alte Schulkameraden und genieß das Internet wie früher. ✨",
];

// Idempotent: postet Fidolins Begrüßung auf die Pinnwand eines Users, falls noch nicht geschehen.
export function welcomeNewUserIfNeeded(targetUserId) {
  if (!targetUserId) return false;
  let fidolinId;
  try { fidolinId = ensureFidolinUser(); } catch { return false; }
  if (fidolinId === targetUserId) return false;
  const already = db().prepare(
    "SELECT id FROM pinnwand WHERE target_user_id = ? AND from_user_id = ? LIMIT 1"
  ).get(targetUserId, fidolinId);
  if (already) return false;
  const u = getUserById(targetUserId);
  if (!u) return false;
  const name = u.displayName || u.display_name || u.username || "du";
  const tmpl = FIDOLIN_WELCOME_TEXTS[Math.floor(Math.random() * FIDOLIN_WELCOME_TEXTS.length)];
  const text = tmpl.replace("{name}", name);
  try {
    addPinnwand(targetUserId, fidolinId, text);
    return true;
  } catch {
    return false;
  }
...
export function getBuschfunk(limit = 30) {
  ...
  // Boosted-Posts oben — sortiert nach Boost-Ende, danach Rest nach Zeit.
  const now = Date.now();
...
// Alles auf einmal als gelesen: Konversationen + Räume.
export function markAllChatsAndRoomsRead(userId) {
  let chats = 0, rooms = 0;
  for (const c of getConversationsForUser(userId)) { markConversationRead(userId, c.partnerId); chats++; }
  for (const r of listMyChatRooms(userId)) { markRoomRead(r.id, userId); rooms++; }
  return { chats, rooms };
}
...
// Position muss frisch sein (< 10 Min), gestorbene Pets nicht zeigen.
...
db().exec(`
  CREATE TABLE IF NOT EXISTS tipspiel_matches (
    match_id INTEGER PRIMARY KEY, season TEXT NOT NULL, matchday INTEGER NOT NULL,
    team1 TEXT NOT NULL, team2 TEXT NOT NULL,
    team1_icon TEXT DEFAULT '', team2_icon TEXT DEFAULT '',
    kickoff_at INTEGER NOT NULL, goals1 INTEGER, goals2 INTEGER,
    finished INTEGER NOT NULL DEFAULT 0, settled INTEGER NOT NULL DEFAULT 0
  );
  CREATE INDEX IF NOT EXISTS idx_tip_matches_md ON tipspiel_matches (season, matchday);
  CREATE TABLE IF NOT EXISTS tipspiel_tips (
    user_id INTEGER NOT NULL, match_id INTEGER NOT NULL,
    tip1 INTEGER NOT NULL, tip2 INTEGER NOT NULL,
    points INTEGER NOT NULL DEFAULT 0, at INTEGER NOT NULL,
    PRIMARY KEY (user_id, match_id)
  );
  CREATE INDEX IF NOT EXISTS idx_tips_match ON tipspiel_tips (match_id);
  CREATE TABLE IF NOT EXISTS tipspiel_entries (
    user_id INTEGER NOT NULL, season TEXT NOT NULL, matchday INTEGER NOT NULL,
    paid_at INTEGER NOT NULL, PRIMARY KEY (user_id, season, matchday)
  );
  CREATE TABLE IF NOT EXISTS tipspiel_pots (
    season TEXT NOT NULL, matchday INTEGER NOT NULL,
    pot INTEGER NOT NULL DEFAULT 0, paid_out_at INTEGER,
    PRIMARY KEY (season, matchday)
  );
`);

export const TIPSPIEL_ENTRY_COST = 50;

function tipspielScore(t1, t2, g1, g2) {
  if (t1 === g1 && t2 === g2) return 3;
  if (t1 - t2 === g1 - g2 && t1 - t2 !== 0) return 2;
  if (t1 - t2 === 0 && g1 - g2 === 0) return 2;
  const tw = t1 > t2 ? 1 : t1 < t2 ? 2 : 0;
  const rw = g1 > g2 ? 1 : g1 < g2 ? 2 : 0;
  return tw === rw && rw !== 0 ? 1 : 0;
}

export function tipspielUpsertMatches(matches) {
  if (!matches || !matches.length) return 0;
  const stmt = db().prepare(`
    INSERT INTO tipspiel_matches (match_id, season, matchday, team1, team2, team1_icon, team2_icon, kickoff_at, goals1, goals2, finished)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(match_id) DO UPDATE SET
      season=excluded.season, matchday=excluded.matchday,
      team1=excluded.team1, team2=excluded.team2,
      team1_icon=excluded.team1_icon, team2_icon=excluded.team2_icon,
      kickoff_at=excluded.kickoff_at,
      goals1=excluded.goals1, goals2=excluded.goals2, finished=excluded.finished
  `);
  db().transaction((items) => {
    for (const m of items) {
      stmt.run(m.matchId, m.season, m.matchday, m.team1, m.team2, m.team1Icon || "", m.team2Icon || "",
        m.kickoffAt || 0,
        m.goals1 == null ? null : Number(m.goals1),
        m.goals2 == null ? null : Number(m.goals2),
        m.finished ? 1 : 0);
    }
  })(matches);
...
export function tipspielSettleAndPayout() {
  const ready = db().prepare("SELECT match_id, season, matchday, goals1, goals2 FROM tipspiel_matches WHERE finished = 1 AND settled = 0").all();
  for (const m of ready) {
    const tips = db().prepare("SELECT user_id, tip1, tip2 FROM tipspiel_tips WHERE match_id = ?").all(m.match_id);
    const update = db().prepare("UPDATE tipspiel_tips SET points = ? WHERE user_id = ? AND match_id = ?");
    db().transaction(() => {
      for (const t of tips) update.run(tipspielScore(t.tip1, t.tip2, m.goals1, m.goals2), t.user_id, m.match_id);
      db().prepare("UPDATE tipspiel_matches SET settled = 1 WHERE match_id = ?").run(m.match_id);
    })();
  }
  const mdRows = db().prepare(`
    SELECT season, matchday FROM tipspiel_matches
     GROUP BY season, matchday
    HAVING SUM(CASE WHEN finished=1 AND settled=1 THEN 1 ELSE 0 END) = COUNT(*) AND COUNT(*) > 0
  `).all();
  for (const md of mdRows) {
    const pot = db().prepare("SELECT pot, paid_out_at FROM tipspiel_pots WHERE season = ? AND matchday = ?").get(md.season, md.matchday);
    if (!pot || pot.paid_out_at || pot.pot <= 0) continue;
    const lb = db().prepare(`
      SELECT t.user_id, SUM(t.points) AS pts FROM tipspiel_tips t
      JOIN tipspiel_matches m ON m.match_id = t.match_id
      WHERE m.season = ? AND m.matchday = ?
      GROUP BY t.user_id ORDER BY pts DESC
    `).all(md.season, md.matchday);
    if (!lb.length) continue;
    const top = lb[0].pts;
    const winners = lb.filter((x) => x.pts === top);
    const share = Math.floor(pot.pot / winners.length);
    db().transaction(() => {
      for (const w of winners) {
        if (share > 0) awardCredits(w.user_id, share, "tipspiel_win", { type: "tipspiel_md", id: md.season + "-" + md.matchday });
      }
      db().prepare("UPDATE tipspiel_pots SET paid_out_at = ? WHERE season = ? AND matchday = ?").run(Date.now(), md.season, md.matchday);
    })();
  }
}

export function tipspielCurrentMatchday() {
  const r = db().prepare("SELECT season, matchday FROM tipspiel_matches WHERE finished = 0 ORDER BY kickoff_at ASC LIMIT 1").get();
  return r || db().prepare("SELECT season, matchday FROM tipspiel_matches ORDER BY kickoff_at DESC LIMIT 1").get() || null;
}

export function tipspielListMatchday(season, matchday, userId) {
  const matches = db().prepare(`
    SELECT match_id AS matchId, team1, team2, team1_icon AS team1Icon, team2_icon AS team2Icon,
           kickoff_at AS kickoffAt, goals1, goals2, finished, settled
    FROM tipspiel_matches WHERE season = ? AND matchday = ? ORDER BY kickoff_at ASC
  `).all(season, matchday);
  const tips = userId ? db().prepare(`
    SELECT match_id AS matchId, tip1, tip2, points FROM tipspiel_tips
    WHERE user_id = ? AND match_id IN (SELECT match_id FROM tipspiel_matches WHERE season = ? AND matchday = ?)
  `).all(userId, season, matchday) : [];
  const tipMap = Object.fromEntries(tips.map((t) => [t.matchId, t]));
  return matches.map((m) => Object.assign({}, m, { myTip: tipMap[m.matchId] || null }));
}

export function tipspielHasEntry(userId, season, matchday) {
  return !!db().prepare("SELECT 1 FROM tipspiel_entries WHERE user_id = ? AND season = ? AND matchday = ?").get(userId, season, matchday);
}

export function tipspielBuyEntry(userId, season, matchday) {
  if (tipspielHasEntry(userId, season, matchday)) return { ok: true, alreadyPaid: true };
  const s = spendCredits(userId, TIPSPIEL_ENTRY_COST, "tipspiel_entry", { type: "tipspiel_md", id: season + "-" + matchday });
  if (!s.ok) return { ok: false, balance: s.balance, missing: s.missing };
  db().transaction(() => {
    db().prepare("INSERT OR IGNORE INTO tipspiel_entries (user_id, season, matchday, paid_at) VALUES (?, ?, ?, ?)").run(userId, season, matchday, Date.now());
    db().prepare("INSERT INTO tipspiel_pots (season, matchday, pot) VALUES (?, ?, ?) ON CONFLICT(season, matchday) DO UPDATE SET pot = pot + excluded.pot").run(season, matchday, TIPSPIEL_ENTRY_COST);
  })();
  return { ok: true, alreadyPaid: false };
}

export function tipspielPlaceTip(userId, matchId, tip1, tip2) {
  const m = db().prepare("SELECT season, matchday, kickoff_at, finished FROM tipspiel_matches WHERE match_id = ?").get(matchId);
  if (!m) return { ok: false, error: "Match nicht gefunden" };
  if (m.finished || Date.now() >= m.kickoff_at) return { ok: false, error: "Anpfiff vorbei - kein Tipp mehr moeglich" };
  if (!tipspielHasEntry(userId, String(m.season), m.matchday)) return { ok: false, error: "Erst Spieltag-Eintritt fuer " + TIPSPIEL_ENTRY_COST + " Vibes kaufen" };
  const t1 = Math.max(0, Math.min(20, Number(tip1) || 0));
  const t2 = Math.max(0, Math.min(20, Number(tip2) || 0));
  db().prepare("INSERT INTO tipspiel_tips (user_id, match_id, tip1, tip2, points, at) VALUES (?, ?, ?, ?, 0, ?) ON CONFLICT(user_id, match_id) DO UPDATE SET tip1 = excluded.tip1, tip2 = excluded.tip2, at = excluded.at").run(userId, matchId, t1, t2, Date.now());
  return { ok: true };
}

export function tipspielPot(season, matchday) {
  return db().prepare("SELECT pot, paid_out_at AS paidOutAt FROM tipspiel_pots WHERE season = ? AND matchday = ?").get(season, matchday) || { pot: 0, paidOutAt: null };
}

export function tipspielLeaderboardSeason(season, limit) {
  return db().prepare(`
    SELECT u.id AS userId, u.username, u.display_name AS displayName, u.avatar_url AS avatarUrl,
           SUM(t.points) AS points, COUNT(t.match_id) AS tips
    FROM tipspiel_tips t JOIN tipspiel_matches m ON m.match_id = t.match_id
    JOIN users u ON u.id = t.user_id
    WHERE m.season = ? GROUP BY u.id ORDER BY points DESC, tips DESC LIMIT ?
  `).all(season, limit || 20);
}

export function tipspielLeaderboardMatchday(season, matchday, limit) {
  return db().prepare(`
    SELECT u.id AS userId, u.username, u.display_name AS displayName, u.avatar_url AS avatarUrl,
           SUM(t.points) AS points
    FROM tipspiel_tips t JOIN tipspiel_matches m ON m.match_id = t.match_id
    JOIN users u ON u.id = t.user_id
    WHERE m.season = ? AND m.matchday = ? GROUP BY u.id ORDER BY points DESC LIMIT ?
  `).all(season, matchday, limit || 10);
}

// ============================================================
// SHIELD: Schutz-Einstellungen fuer Frauen (opt-in)
// ============================================================
export function getShieldSettings(userId) {
  const row = db().prepare(`
    SELECT shield_enabled AS enabled, shield_msg_policy AS msgPolicy,
           shield_min_age_days AS minAgeDays, shield_word_filter AS wordFilter
      FROM users WHERE id = ?
  `).get(userId);
  return {
    enabled: !!(row && row.enabled),
    msgPolicy: (row && row.msgPolicy) || "all",
    minAgeDays: (row && row.minAgeDays) || 0,
    wordFilter: !!(row && row.wordFilter),
  };
}
export function setShieldSettings(userId, s) {
  const enabled = s.enabled ? 1 : 0;
  const msgPolicy = ["all","friends","women","none"].includes(s.msgPolicy) ? s.msgPolicy : "all";
  const minAgeDays = Math.max(0, Math.min(365, Number(s.minAgeDays) || 0));
  const wordFilter = s.wordFilter ? 1 : 0;
  db().prepare("UPDATE users SET shield_enabled=?, shield_msg_policy=?, shield_min_age_days=?, shield_word_filter=? WHERE id=?")
    .run(enabled, msgPolicy, minAgeDays, wordFilter, userId);
  return getShieldSettings(userId);
}
