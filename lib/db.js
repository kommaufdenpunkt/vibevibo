...
// Auth
// ============================================================
export function createUser({ username, displayName, password, emoji, regIp }) {
  ...
  const info = db().prepare(`
    INSERT INTO users (username, display_name, password_hash, emoji, mood, about_me, interests, bg_music, created_at, last_seen, status, reg_ip)
    VALUES (?, ?, ?, ?, '', 'Heyhey, ich bin neu bei VibeVibo!', '[]', '', ?, ?, 'pending', ?)
  `).run(cleaned, displayName || cleaned, hash, emoji || "🙂", now, now, regIp || "");
...
// Profilbilder-Moderation
export function submitAvatar(userId, dataUrl, status, reason) {
  db().prepare("UPDATE users SET avatar_url = ?, avatar_status = ?, avatar_submitted_at = ?, avatar_reason = ? WHERE id = ?")
    .run(dataUrl, status || "pending", Date.now(), reason || "", userId);
}

export function setAvatarStatus(userId, status, reason) {
  if (!["none", "pending", "approved", "rejected"].includes(status)) return;
  db().prepare("UPDATE users SET avatar_status = ?, avatar_reason = ? WHERE id = ?").run(status, reason || "", userId);
}

export function listPendingAvatars() {
  return db().prepare("SELECT * FROM users WHERE avatar_status = 'pending' ORDER BY avatar_submitted_at ASC")
    .all().map((u) => ({ ...userRow(u), avatarUrlRaw: u.avatar_url || "", avatarSubmittedAt: u.avatar_submitted_at || 0 }));
}

export function listRejectedAvatars() {
  return db().prepare("SELECT * FROM users WHERE avatar_status = 'rejected' ORDER BY avatar_submitted_at DESC")
    .all().map((u) => ({ ...userRow(u), avatarUrlRaw: u.avatar_url || "", avatarReason: u.avatar_reason || "" }));
}

export function getRawAvatar(userId) {
  const r = db().prepare("SELECT avatar_url, avatar_status FROM users WHERE id = ?").get(userId);
  return r ? { url: r.avatar_url || "", status: r.avatar_status || "none" } : null;
}

// Avatare, die seit > maxAgeMs in Prüfung hängen, automatisch freigeben (Fidolin-SLA)
export function autoApproveStaleAvatars(maxAgeMs) {
  const cutoff = Date.now() - maxAgeMs;
  const rows = db().prepare("SELECT id FROM users WHERE avatar_status = 'pending' AND avatar_submitted_at > 0 AND avatar_submitted_at < ?").all(cutoff);
  for (const r of rows) {
    db().prepare("UPDATE users SET avatar_status = 'approved', avatar_reason = 'Auto-Freigabe (Fidolin-Frist abgelaufen)' WHERE id = ?").run(r.id);
    logMod({ userId: r.id, kind: "avatar", decision: "approved", reason: "Auto-Freigabe nach Frist", by: "fidolin" });
  }
  return rows.length;
}
...
export function adminStats() {
  ...
  return {
    ...
    pendingAvatars: d.prepare("SELECT COUNT(*) AS n FROM users WHERE avatar_status = 'pending'").get().n,
  };
}
...
export function getRecentVisitors(targetUserId, limit = 12) {
  // Letzter Besuch pro Besucher, neueste zuerst
  return db().prepare(`
    SELECT u.username, u.display_name AS displayName, u.emoji, u.last_seen AS lastSeen,
...
export function getBuschfunk(limit = 30) {
  ...
  for (const p of d.prepare(`
    SELECT p.created_at AS at, f.username AS au, f.display_name AS an, f.emoji AS ae,
  ...
  `).all(limit)) {
    events.push({ type: "pinnwand", at: p.at, actor: { username: p.au, displayName: p.an, emoji: p.ae },
      target: { username: p.tu, displayName: p.tn }, detail: p.detail });
  }

  for (const g of d.prepare(`
    SELECT g.created_at AS at, g.gift_id AS gift, f.username AS au, f.display_name AS an, f.emoji AS ae,
  ...
  `).all(limit)) {
    events.push({ type: "gift", at: g.at, gift: g.gift, actor: { username: g.au, displayName: g.an, emoji: g.ae },
      target: { username: g.tu, displayName: g.tn } });
  }

  for (const gp of d.prepare(`
    SELECT gp.created_at AS at, u.username AS au, u.display_name AS an, u.emoji AS ae,
  ...
  `).all(limit)) {
    events.push({ type: "grouppost", at: gp.at, actor: { username: gp.au, displayName: gp.an, emoji: gp.ae },
      group: { slug: gp.gslug, name: gp.gname }, detail: gp.detail });
  }

  for (const u of d.prepare(`
    SELECT created_at AS at, username AS au, display_name AS an, emoji AS ae
      FROM users ORDER BY created_at DESC LIMIT 10
  `).all()) {
    events.push({ type: "newuser", at: u.at, actor: { username: u.au, displayName: u.an, emoji: u.ae } });
  }
