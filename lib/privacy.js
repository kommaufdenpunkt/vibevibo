...
export function canMessage(senderId, recipientId) {
  ...
  // 🛡 Women-Shield: verified_only_dm — nur stimm-verifizierte dürfen schreiben
  if (typeof DB.getWomenShieldFields === "function") {
    const ws = DB.getWomenShieldFields(recipientId);
    if (ws?.verified_only_dm) {
      const senderVerified = typeof DB.isGenderVerified === "function"
        ? DB.isGenderVerified(senderId) : false;
      if (!senderVerified) {
        return {
          ok: false,
          reason: "Dieser Nutzer empfängt nur Nachrichten von stimm-verifizierten Accounts. Du kannst dich unter „Privatsphäre → Stimm-Verifikation“ verifizieren lassen.",
        };
      }
    }
  }
  ...
  // direkt anschreiben. Erst kommentieren, dann darf sie zuerst antworten.
  if (typeof DB.isWomenInitiativeBlocking === "function"
      && DB.isWomenInitiativeBlocking(senderId, recipientId)) {
    return {
      ok: false,
      reason: "🩷 Diese Nutzerin startet Gespräche selbst. Reagiere oder kommentiere auf ihre Posts — wenn sie sich für dich interessiert, schreibt sie dir.",
    };
  }
...
export function privacyStatusForUser(user) {
  ...
  const verifiedOnlyDm = user?.verifiedOnlyDm ?? user?.verified_only_dm ?? 0;
  const liveStrictMode = user?.liveStrictMode ?? user?.live_strict_mode ?? 0;
  const womenInitiative = user?.womenInitiative ?? user?.women_initiative ?? 0;
  ...
  return {
    ...
    verifiedOnlyDm: !!verifiedOnlyDm,
    liveStrictMode: !!liveStrictMode,
    womenInitiative: !!womenInitiative,
  };
}
